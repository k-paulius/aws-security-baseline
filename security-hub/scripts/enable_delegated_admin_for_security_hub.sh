...
if [ -n "$DELEGATION_DATE" ]; then
    ...
    echo "- NOTE:"
    echo "   This will enable AWS Security Hub as a Trusted Service in Organizations, register a delegated administrator"
    echo "   account for Security Hub in Organizations and designate the Security Hub administrator account in this region."
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        ...
        aws securityhub enable-organization-admin-account \
            --admin-account-id=$DEL_ADMIN_ACCOUNT_ID
        echo "- Done."
    else
        echo "- Exiting."
    fi
