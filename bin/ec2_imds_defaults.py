...
# desired default instance metadata service (IMDS) settings
# only settings specified in the dictionary will be modified
#
# Allowed keys and values"
#   HttpTokens              = 'optional' (v1 and v2)|'required' (v2 only)|'no-preference',
#   HttpPutResponseHopLimit = 123|-1 (for no preference),
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Modify the default IMDS settings at the account level across organization accounts and regions.')
    ...
    parser.add_argument('--dry-run', action='store_true', help='Print current default IMDS settings at the account level without making changes.')
        ...
        for account in accounts:
            ...
            for region in regions:
                try:
                    ...
                    current_config = response['AccountLevel']
                    config_changed = False

                    for key in INSTANCE_METADATA_DEFAULTS:
                        if ((key not in current_config and INSTANCE_METADATA_DEFAULTS[key] != 'no-preference' and INSTANCE_METADATA_DEFAULTS[key] != -1) or
                            (key in current_config and INSTANCE_METADATA_DEFAULTS[key] != current_config[key])
                        ):
                            config_changed = True
                            break

                    if args.dry_run:
                        logger.info(f'{account["Id"]} / {region}: Default IMDS settings at the account level {"do not match" if config_changed else "match"} the desired state. Current configuration - {current_config}')
                    else:
                        if not config_changed:
                            logger.info(f'{account["Id"]} / {region}: Default IMDS settings at the account level already match the desired state')
                        else:
                            ec2_client.modify_instance_metadata_defaults(**INSTANCE_METADATA_DEFAULTS)
                            logger.info(f'{account["Id"]} / {region}: Modified default IMDS settings at the account level')
                except ClientError as e:
