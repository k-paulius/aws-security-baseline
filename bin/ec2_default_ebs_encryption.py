...
def main():
    ...
    parser = argparse.ArgumentParser(description='Enable default EBS encryption across organization accounts and regions.')
    ...
    parser.add_argument('--dry-run', action='store_true', help='Print current EBS encryption by default status without making changes.')
        ...
        for account in accounts:
            ...
            for region in regions:
                try:
                    ...
                    ebs_encryption = response['EbsEncryptionByDefault']

                    if args.dry_run:
                        logger.info(f'{account["Id"]} / {region} : EBS encryption by default is {"enabled" if ebs_encryption else "disabled"}')
                    else:
                        if ebs_encryption:
                            logger.info(f'{account["Id"]} / {region} : EBS encryption by default is already enabled')
                        else:
                            ec2_client.enable_ebs_encryption_by_default()
                            logger.info(f'{account["Id"]} / {region} : Enabled EBS encryption by default')
                except ClientError as e:
