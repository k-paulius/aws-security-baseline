...
# desired block public access for AMIs configuration
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Enable block public access for AMIs across organization accounts and regions.')
    ...
    parser.add_argument('--dry-run', action='store_true', help='Print current block public access for AMIs status without making changes.')
        ...
        for account in accounts:
            ...
            for region in regions:
                try:
                    ...
                    response = ec2_client.get_image_block_public_access_state()
                    current_config = response['ImageBlockPublicAccessState']
                    config_changed = True

                    if current_config == IMAGE_BLOCK_PUBLIC_ACCESS_STATE:
                        config_changed = False

                    if args.dry_run:
                        logger.info(f'{account["Id"]} / {region}: Block public access for AMIs {"does not match" if config_changed else "matches"} the desired state. Current configuration - {current_config}')
                    else:
                        if not config_changed:
                            logger.info(f'{account["Id"]} / {region}: Block public access for AMIs already matches the desired state')
                        else:
                            ec2_client.enable_image_block_public_access(
                                ImageBlockPublicAccessState=IMAGE_BLOCK_PUBLIC_ACCESS_STATE
                            )
                            logger.info(f'{account["Id"]} / {region}: Enabled block public access for AMIs')
                except ClientError as e:
